...
Other forms:
  ...
  --install [--path] write the guidance to ~/.cueckoo/common-guidance.md
                     (override the location with --path). Force-overwrites
                     regardless of the file's current state
  --check   [--path] verify the on-disk guidance matches what this binary
                     would write; exit non-zero on drift. Intended for use
                     in CI / pre-mail gates (strict byte equality)

The routine refresh logic — write the file when missing, refresh it
on a cueckoo version bump, and preserve same-version local edits —
is in "cueckoo version update", which is what the Claude Code
SessionStart hook should invoke.
`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			case hashOnly:
				...
				return err
			case install:
				return runGuidanceInstall(path)
			case check:
				return runGuidanceCheck(path)
			default:
	...
	cmd.Flags().BoolVar(&install, "install", false, "force-write the guidance to ~/.cueckoo/common-guidance.md (or --path)")
	cmd.Flags().BoolVar(&check, "check", false, "verify the on-disk guidance file matches; exit non-zero on drift")
	cmd.Flags().StringVar(&path, "path", "", "override the default ~/.cueckoo/common-guidance.md path for --install or --check")
	return cmd
}

// resolveGuidancePath returns override if non-empty, otherwise the
// default ~/.cueckoo/common-guidance.md path.
func resolveGuidancePath(override string) (string, error) {
	if override != "" {
		return override, nil
	}
...
func runGuidanceInstall(override string) error {
	p, err := resolveGuidancePath(override)
	if err != nil {
		return err
	}
...
func runGuidanceCheck(override string) error {
	p, err := resolveGuidancePath(override)
	if err != nil {
		return err
	}
