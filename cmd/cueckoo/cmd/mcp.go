...
The change argument can be:
  ...
  git:<ref>            — any git ref, e.g. git:HEAD`,
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "gerrit_lineage",
		Description: `Show the local commit history for a Gerrit change.

Resolves a change identifier to its Change-Id, then searches local
git reflogs for all commits that carry that Change-Id in their
trailers. This shows how a change evolved locally over time — each
amend or rebase creates a new commit SHA but preserves the Change-Id.

The search is scoped to branches sharing the same upstream target as
the current branch. This matches Gerrit's view that a review is
defined by Target Branch + Change-Id. If the same Change-Id was
cherry-picked to a different target (e.g. a release branch), those
commits are excluded — they belong to a different Gerrit review.
Commits from other local branches that track the same upstream are
included, even if you have since switched branches.

This is particularly useful after addressing review feedback: by
diffing between two versions of the same change (e.g.
git diff <old-sha> <new-sha>) you can see exactly what changed
relative to the previous version, confirming that the feedback was
addressed correctly.
...
Note: this only shows local history from the git reflog (typically 90
days for unreachable entries). The local lineage and the Gerrit patchset
history can differ — not all local commits are mailed to Gerrit, and
other contributors may push patchsets that do not appear locally.`,
	}, handleGerritLineage)
