...
The argument can be a change number (e.g. 1233340), a Change-Id
(e.g. Ia15e97465869aa18ba2b8c9795cec18f438d7b76), or a git ref (e.g. HEAD, HEAD~2, a commit SHA).
...
Use --unresolved-only to show only unresolved threads.`,
...
repo's CLAUDE.md file. Use this to check whether a repo's CLAUDE.md is
up to date with the latest shared conventions (commit messages, code
review, community support, etc.).
...
type gerritCommentsInput struct {
	Change         string `json:"change" jsonschema:"change number, Change-Id, or git ref"`
