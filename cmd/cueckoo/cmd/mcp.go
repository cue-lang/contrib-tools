...
unresolved to false for FYI or informational comments that do not require
a response.`,
...
type gerritDraftCommentInput struct {
	...
	Unresolved *bool  `json:"unresolved,omitempty" jsonschema:"whether the comment requires action from the CL author (default true). Set to false for FYI/informational comments."`
...
func handleGerritDraftComment(ctx context.Context, req *mcp.CallToolRequest, input gerritDraftCommentInput) (*mcp.CallToolResult, any, error) {
	unresolved := true
	if input.Unresolved != nil {
		unresolved = *input.Unresolved
	}
	result, err := postGerritDraftComment(input.Change, input.PatchSet, input.Path, input.Line, unresolved, input.Message)
	if err != nil {
		return &mcp.CallToolResult{
			Content: []mcp.Content{
				&mcp.TextContent{Text: fmt.Sprintf("error: %v", err)},
