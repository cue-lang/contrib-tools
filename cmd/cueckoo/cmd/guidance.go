...
stage them, and amend with git codereview change. Then resume:
...
To amend both code and message together, use GIT_EDITOR with a
script that edits the message file in place:

    git add <files>
    GIT_EDITOR="sed -i '1s/.*/cmd\/foo: updated summary/'" \
      git commit --amend

This opens the existing message (including the Change-Id) in the
editor script, which modifies only what it needs to. The Change-Id
is preserved because the script edits the file rather than
replacing it.

For more complex message edits, write a small script:

    git add <files>
    GIT_EDITOR=/tmp/edit_msg.sh git commit --amend

where /tmp/edit_msg.sh edits "$1" (the message file) in place.
...
Only use -m or -F if you manually include the original Change-Id
in the new message, which is error-prone. The GIT_EDITOR approach
is always safer because it edits the existing message in place.
