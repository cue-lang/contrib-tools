...
Substitute the repo's default branch where it differs (origin/main
on some repos). Naming the base does two things: it starts the
branch at the remote tip rather than at wherever HEAD happens to be
— a bare "git checkout -b my-branch" silently branches from the
current commit, which on a detached HEAD is very unlikely to be the
intended base — and it sets the branch's upstream to that base.
Vary the starting point only when context explicitly directs
otherwise (e.g. when building on someone else's branch).
...
- Create a work branch off the upstream's default branch
  (origin/master for CUE repos) — see "Work branches and upstreams"
  above:
...
to the local branch name — not from @{u} — so an upstream of
origin/main is fine and no extra flags are needed. Two caveats:
