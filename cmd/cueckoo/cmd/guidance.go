...
Key commands:
    ...
    git codereview mail [-r reviewer,...] [-cc mail,...] [-f]
                        [-trybot] [-autosubmit] [-wip] [revision]
      ...
      Push pending change to Gerrit for review. Flags:
        ...
        -trybot      set Commit-Queue+1 (starts CI)
...
- Revise after feedback: make changes, stage them, then
  git codereview change (amends the commit), then
  git codereview mail
...
- After making changes: git codereview change then
  git codereview mail to send the updated patchset
