...
- For the top commit (outside a rebase): use git codereview reword,
  or run git commit --amend with cueckoo rewrite-commit-msg as
  GIT_EDITOR. Both preserve the Change-Id. Do not use
  git codereview change -m or git commit --amend -m, which replace
  the message entirely and cause the hooks to generate a new
  Change-Id.
...
is already the target: edit the working tree and run
git codereview change to amend.
