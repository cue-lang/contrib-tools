...
- All commits must include a Change-Id trailer (added automatically
  ...
  uniquely identify a change — see "Preserving Change-Ids" below
...
the full stack. If the target commit is not at the top, use
git codereview rebase-work and mark it as "edit" before making
changes. Only when the target commit is already at HEAD can you edit
the working tree and amend directly with git codereview change.

After editing:
- If you used rebase-work: git rebase --continue
- If you amended HEAD directly: git codereview change
