...
func resolveChangeNumber(arg string) (string, error) {
	...
	default:
		return "", fmt.Errorf("unknown change prefix %q, must be cl:, changeid:, or git:", prefix)
	}
}
...
func gerritAPI(path string) ([]byte, error) {
	...
	// Look up credentials from ~/.netrc for the Gerrit host.
...
// netrcCredentials looks up login and password for the given machine
// in ~/.netrc. Returns ("", "", false) if not found.
func netrcCredentials(machine string) (login, password string, ok bool) {
